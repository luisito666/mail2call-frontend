...
import { Trigger, TriggerCreate, TriggerUpdate, ContactGroup, PaginatedResponse } from '../../models/interfaces';
      ...
      @if (paginationInfo().total_pages > 1) {
        <div class="pagination-container">
          <div class="pagination-info">
            ...
            de {{ paginationInfo().total }} triggers
          </div>
...
export class TriggersComponent implements OnInit {
  ...
  paginationInfo = signal<PaginatedResponse<Trigger>>({
    items: [],
    total: 0,
    page: 1,
    per_page: 10,
  ...
  loadData(page?: number, per_page?: number): void {
    ...
    Promise.all([
      this.triggerService.getTriggers(currentPage, currentPerPage).toPromise(),
      this.contactGroupService.getAllContactGroups().toPromise()
    ]).then(([triggerResponse, groups]) => {
      if (triggerResponse) {
        this.paginationInfo.set(triggerResponse);
        this.triggers.set(triggerResponse.items);
      }
